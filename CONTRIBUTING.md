...
Settings UI in a browser (no Rust build, no real config): `npm run dev:mock` serves `index.html` at `http://localhost:1420` with a fake Tauri bridge (`dev/mock-tauri.js`, dev server only, never in `dist/`). Pick a state with `?scenario=configured|fresh|update|errors`, add `&fail=config` to make loading fail or `&accent=%23a550a7` to simulate a macOS accent color, and poke it from the console through `window.__selaraMock` (`emit`, `calls`, `state`; native menus are recorded in `menu` and picked with `chooseMenuItem(text)`, and `state.confirm = false` answers native confirmations with Cancel). `npm run e2e` starts its own mock server on port 1430 (so it never reuses a `npm run dev` or `tauri dev` server on 1420) and runs the local Playwright WebKit smoke suite at the default and minimum window sizes in light and dark, and writes screenshots and a native-feel style report to `e2e/artifacts/` (first run: `npx playwright install webkit`). Headless WebKit does not render `backdrop-filter`, and the browser has no vibrancy or title bar, so confirm visual changes in `npx tauri dev` too.
