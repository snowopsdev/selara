...
- Merging that pull request creates the `vX.Y.Z` tag and the GitHub Release, and attaches a macOS build of the `selara` CLI, a DMG of the Tauri app, and a rendered Homebrew cask and formula (`homebrew/*.tmpl` via `scripts/release/render-homebrew.sh`). Signing and notarization run when the `APPLE_*` repository secrets are set (see the comment block above the `desktop` job in `.github/workflows/release.yml`); without them the DMG is ad-hoc signed. `HOMEBREW_TAP_TOKEN` enables the push to `snowopsdev/homebrew-selara`.
